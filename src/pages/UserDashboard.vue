...
const fetchJadwalToday = async () => {
  try {
    ...
    if (res.data.success && res.data.data.length > 0) {
      ...
      // cek apakah jadwal today ada di jadwal week
      canAjukan.value = jadwalList.value.some((j) => j.id === jadwalToday.value.id)
    } else {
      jadwalToday.value = null
      canAjukan.value = false
    }
  } catch (err) {
