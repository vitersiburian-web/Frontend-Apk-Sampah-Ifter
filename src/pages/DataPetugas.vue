<template>
  <q-page class="q-pa-md bg-green-1">
    ...
    <q-list separator class="data-list">
      <q-item-label header class="text-weight-bold text-grey-8">Daftar Petugas ({{ filteredPetugas.length }})</q-item-label>
      
      <q-item v-for="petugas in filteredPetugas" :key="petugas.id" class="q-py-md bg-white data-item">
        <q-item-section avatar>
            <q-icon name="support_agent" color="primary" size="sm" />
        </q-item-section>
        
        <q-item-section>
          <q-item-label class="text-weight-bold text-dark text-lg">{{ petugas.name }}</q-item-label>
          <q-item-label caption>{{ petugas.phone }}</q-item-label>
          <q-item-label caption>Status: 
             <q-badge :color="petugas.status === 'Aktif' ? 'green-7' : 'red-5'" :label="petugas.status" class="status-badge" />
          </q-item-label>
        </q-item-section>
        
        <q-item-section side>
          <q-btn label="Shift" size="sm" color="yellow-8" text-color="black" unelevated dense class="q-px-sm q-mb-xs rounded-btn" />
        </q-item-section>

        <q-item-section side>
          <q-btn icon="edit" size="sm" flat color="blue" dense @click="editItem(petugas)" class="q-mb-xs" />
...
const deleteItem = (id) => {
    $q.dialog({
        ...
        message: 'Apakah Anda yakin ingin menghapus data petugas ini?',
        cancel: true,
        persistent: true
    }).onOk(() => {
        if (deletePetugas(id)) {
            $q.notify({ message: 'Data petugas berhasil dihapus!', color: 'positive' });
        }
    });
};
</script>
